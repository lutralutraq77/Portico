...
- Each data stream has a separate, continuously read cancellation Watch RPC bound to the same TLS leaf and a random correlation ID. Both watches must be ready before forwarding. Runtime tests found that ending a data RPC can leave its terminal status behind unread payload; the separate flow-control window carries closure without draining or discarding an unbounded payload queue. Watch loss closes the corresponding data stream. [Implemented carrier contract](docs/carrier-protocol.md).
