...
Status: Phases 6 and 7 are active. The Linux/Arch client, encrypted enrollment, protected local agent, installed user service and application launcher have recorded qualification. Real issuer/curl HTTPS integration passed all six hosted workflows on 1c1bd60, including nine HTTPS cases. Phase 7 includes a qualified inventory API and a private interface with native race, Linux static analysis and actual browser/mobile/keyboard/revocation checks passed; combined-source hosted tests are next. Desktop/platform key workflows, secure administration/bootstrap and physical/deployment gates remain open. Complete Phases 6 and 7, then wait for approval before Phase 8; see the [delivery ledger](docs/delivery-status.md).
