...
Status: active implementation is Phase 6, the Linux/Arch client. Phases 0–2 have foundation evidence; Phase 3 issuer/admin/recovery software retains physical and deployment gates. Phase 4 policy and Phase 5 connector components have recorded test evidence. Phase 6 resource access, encrypted enrollment, terminal input, Arch packaging, protected IPC and agent catalog/streaming have hosted qualification. The locked daemon and inactive user unit passed hosted qualification; the actual systemd guest fixture passed service lifecycle, applied restrictions and enrolled-identity resource flows. The application launcher also passed all six hosted workflows; real HTTPS integration is undergoing qualification. Desktop/browser/platform workflows remain open. Phase 7 administrator inventory is implemented in [draft PR 5](https://github.com/lutralutraq77/Portico/pull/5), with local race tests and all six hosted workflows passed on a967c62. The current objective is to finish Phases 6 and 7, then wait for approval before Phase 8; see the [delivery ledger](docs/delivery-status.md).
