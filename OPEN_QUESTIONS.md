...
| Q02 | Browser certificate factor, stable private WebAuthn origin, IP-only bootstrap and key-store access | Native companion with server-verifiable device binding; compare native/browser cert alternatives | Identity/frontend/platform reviewers: complete sequence and malicious-origin/local-user tests | Before admin PKI integration and Phase 7 dashboard |
...
| Q08 | step-ca registration-authority/provisioner constraints and admin renewal | No client access to issuer renewal/signing; exact approved profiles and operation authorization | PKI engineer: direct endpoint bypass tests and crash reconciliation | Before Phase 3 issuance |
...
| Q13 | Project license, name collision, repository ownership and private disclosure channel | Resolve before publication; no invented account, email or legal grant | Project owner/maintainers: explicit governance/licensing choice | Before public GitHub publication |
