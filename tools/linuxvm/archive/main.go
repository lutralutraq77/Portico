...
func main() {
	if len(os.Args) != 15 {
		panic("usage: archive output init cli-test controller-test pki-test adminauth-test wire-test carrier-test control-test boottime-test workload-test adapter-test issuer-test app")
	}
	...
	for i, name := range []string{"init", "tests/cli", "tests/controller", "tests/pki", "tests/adminauth", "tests/wire", "tests/carrier", "tests/control", "tests/boottime", "tests/workload", "tests/adapter", "tests/issuer", "portico"} {
