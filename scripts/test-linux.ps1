...
try {
 ...
 foreach($package in @('cli','controller','pki','adminauth','wire','carrier')){
 ...
 & go run ./tools/linuxvm/archive/main.go (Join-Path $vm 'tests.cpio.gz') (Join-Path $vm 'init') (Join-Path $vm 'cli') (Join-Path $vm 'controller') (Join-Path $vm 'pki') (Join-Path $vm 'adminauth') (Join-Path $vm 'wire') (Join-Path $vm 'carrier') (Join-Path $vm 'adapter') (Join-Path $vm 'issuer') (Join-Path $vm 'portico')
 ...
 if ($body -notmatch '(?m)^PORTICO_LINUX_ALL_PASS\r?$' -or $body -match 'PORTICO_FAIL|--- FAIL:'){throw 'Linux runtime tests failed; see reports/linux-runtime.log'}
