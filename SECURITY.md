...
Portico is a development foundation, not a supported access service. No production release exists. Do not use this Phase 1 binary to protect resources; it can only report its version/help.
...
## Release gate
Production requires actual runtime acceptance evidence, platform/network qualification, tested recovery/update rollback and independent review. The 91 runtime acceptance cases remain planned in Phase 1. Foundation test results must never be represented as network isolation, certificate authentication or revocation proof.
