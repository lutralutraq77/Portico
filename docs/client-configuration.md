...
Interactive Linux use can append `--prompt` instead. It obtains the unlock passphrase through hidden foreground controlling-terminal input while retaining separate application pipes. The [terminal contract](terminal-unlocking.md) defines bounds, cancellation and restoration. Supplying both secret options rejects.
...
Each copy direction uses one 32 KiB buffer. On abrupt carrier termination or caller cancellation, owned pipes close and unblock both workers. A validated peer FIN starts a drain deadline of at most operation_timeout_ms for any final buffered output, independently of carrier closure. Successful local forwarding releases the final acknowledgment; a later carrier event cannot extend the drain deadline. The independent input half-close remains usable while authorized. Kernel pipe buffers and bytes already delivered to an application cannot be recalled. This finite drain does not renew authority or qualify unresolved physical suspend/revocation timing scenarios. See the current [drain-ordering evidence](fin-drain.md).
