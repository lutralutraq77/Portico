...
| Connector | /api/v1/connector/cancellations | Version, Limit (1–64), WaitMillis (0–1000); returns unacknowledged cancellation targets owned by the original connector certificate |
...
Cancellation batches contain Version, ConnectorCertificateID, PolicyRevision, ObservedAt and Items with SessionID and Reason. Reasons are closed, denied or expired. Targets are ordered and repeatedly delivered until acknowledged; there is no client cursor that could skip a lost response. Empty long polls subscribe before the initial database read, wake on a committed audited change or emergency stop, and then repeat the live certificate and database checks. Rollbacks and read-only checks do not signal a change. Waits are limited to 128 per policy engine and two per certificate across connections. Immediate polls do not occupy wait slots. This bounds sleeping handlers; complete service admission and hostile-load qualification remain required.
