...
Hosted CodeQL and dependency review passed on 518f139. Hosted Windows/Ubuntu quality was still running at this checkpoint. New source and documentation heads require their own hosted result. Earlier carrier qualification on f1f190b remains recorded in the [carrier report](phase-5-report.md).
...
Compose the live workload path: verify inner TLS, check the actual connector, match the locally approved hosting tuple, authorize before dialing and activate before application bytes. Track both actual handles and forwarding workers, enforce request-start deadlines using the native clock, close on revocation or failed renewal, and acknowledge only after closure. Execute the full socket-count, delayed-response, partition, replay, restart, suspend and hostile-load scenarios.
