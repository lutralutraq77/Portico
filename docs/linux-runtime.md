...
The Windows-hosted harness boots a real Linux amd64 kernel under QEMU TCG, with no network devices or host filesystem shares. Its custom, disposable initramfs contains the cross-compiled CLI/domain test binaries and a tiny Go PID 1. SQLite test files live in guest tmpfs. This verifies Linux execution and process-crash behavior; it is not an Ubuntu installation, Linux race run or hardware power-loss test.
