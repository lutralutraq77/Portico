...
Owned mutable input, serialized-key and decrypted-state buffers are cleared. Go, JSON, TLS and age may retain internal allocations; no forensic erasure, swap/crash-dump isolation or same-user process isolation is claimed. This software state is not yet connected to the resource client's identity loader. Desktop unlocking, platform custody, resource-client integration, installed services and the remaining acceptance gates still require implementation and qualification.
