...
All twenty-one expanded real-PTY cases passed in the second local focused guest. The actual enrollment-to-resource command scenario passed seven subcases but failed its final transfer check with EOF before delivery, so that complete local run failed. The later hosted full-suite pass does not turn this local result into a pass.
...
The diagnostic rerun passed all four top-level tests and 34 subcases, with zero failures/skips and `PORTICO_LINUX_TERMINAL_CLIENT_FOCUSED_PASS`. Its actual prompt-client scenario took 381.02 guest seconds. The runtime log is `work/reports/phase-6-terminal-client-diagnostic/runtime.log`, SHA-256 `674a0dfb70fb0c447a55baa48c5b45a23f6bf7ca1adf247d8a696699239411f2`; initramfs SHA-256 is `3031ffd3b6b4eaeed95ca8a8829a6ebd8d9da308dd8f5de52ddfc10e04736498`. Only test diagnostics changed; the production runtime, password cost, deadlines and authorization rules remained unchanged. The earlier local EOF was not reproduced in this rerun or the full hosted run; its original cause remains undetermined.
