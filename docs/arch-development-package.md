...
The first container runs `makepkg` as an unprivileged user, with a read-only root, no capabilities, no new privileges, private temporary storage and only the public input/output artifact directories. A deliberately modified application must fail checksum verification before the original is restored and packaged. The normal check function executes the Linux development binary. The package and installed build-package versions are retained. [Arch makepkg reference](https://man.archlinux.org/man/makepkg.8.en).
...
The workflow and scripts are implemented; actual package preparation, Arch build and install/remove qualification are pending. Cross-compilation and container execution do not qualify real desktop integration, systemd service behavior, user isolation, host clocks, physical suspend, hardware custody or Mullvad. Those remain in the [delivery ledger](delivery-status.md).
