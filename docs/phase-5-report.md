...
Hosted qualification completed on documentation head f1f190b8d0604db3eb24fa8c81ebe2de8ebd0755: [Windows/Ubuntu quality](https://github.com/lutralutraq77/Portico/actions/runs/34161443886), [CodeQL](https://github.com/lutralutraq77/Portico/actions/runs/34161443883) and [dependency review](https://github.com/lutralutraq77/Portico/actions/runs/34161443896) all passed. This verifies the carrier checkpoint, not subsequent control-client changes. The original local artifacts are retained under work/reports/phase-5-carrier-c77fc37 with their recorded hashes.

## Remaining work

Subsequent work implements the controller endpoint for checking the actual inner connector leaf, authenticated hosting snapshots, a strict HTTP control client, durable bounded cancellation delivery and immutable connector reports; see the [policy API](policy-api.md). Those changes need their own source-specific evidence. Next compose them with the exact workload proxy, activation before application bytes, request-start-anchored leases, and receipts after actual socket closure. Tests must observe real destination connection counts, authority changes, delayed responses, controller outages, restart and hostile-load resource bounds. Transport Watch closure alone does not prove controller revocation delivery or an accepted revocation SLA; Q05 remains open.
