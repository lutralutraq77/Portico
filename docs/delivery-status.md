...
| 5: isolated connector | Development Linux command and runtime verified on 7ddb5c8 ([command evidence](phase-5-connector-command-report.md)); source 160911c adds complete CONN-01 hosting/socket and CONN-03 controller-scope scenarios ([isolation evidence](phase-5-isolation-report.md)) | Keep final-source hosted evidence current; CONN-04 revocation/restart/replay, installed service/controller/client packaging, qualified host time services and Windows clock health, remaining socket/protocol scenarios, physical suspend and hostile-load/network tests |
...
| 14: audit/hardening | AUDIT-01, CONN-01 and CONN-03 have complete executable scenarios; other component tests provide partial evidence | All 91 full acceptance scenarios, manual threat-model/custom-code review, independent review and resolution of all critical/high findings |

## Evidence rules

The [canonical acceptance plan](../ACCEPTANCE_TEST_PLAN.md) and [manifest](../tests/acceptance/manifest.json) contain 91 distinct runtime cases. AUDIT-01, CONN-01 and CONN-03 are implemented; 88 remain planned. The manifest links executable scenarios; actual passing evidence is specific to the source and run. CONN-01 executes only in the guarded NIC-less Linux guest, with skips on other hosts reported separately. A policy denial response cannot replace observed destination socket counts. Virtual authenticators cannot establish physical hardware backing or independent custody. Cross-compilation cannot prove a platform's keystore, lifecycle or local-user boundary. A quarantined encrypted restore cannot prove a complete recovery ceremony. These requirements remain part of the objective.
