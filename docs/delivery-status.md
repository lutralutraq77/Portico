...
| 5: isolated connector | Carrier verified locally on c77fc37 and hosted on f1f190b ([evidence](phase-5-report.md)); live connector checks, hosting snapshots, strict control client, bounded cancellation delivery and immutable receipts now implemented ([API](policy-api.md)) | Qualify the new control changes on final source; implement service composition, exact workload proxy and leases; actual destination socket counters and hostile-load/network tests |
