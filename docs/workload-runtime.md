...
Both supervisors observe the paired carrier's independent Done signal. A framing reader can be blocked delivering already-received DATA into an unread application pipe, and a forwarding worker can be blocked writing to a destination. Neither can be relied on to read transport EOF promptly. Carrier termination therefore cancels the workload context and closes owned handles through the existing supervisor, even while resource checks and the lease remain valid. [Overload verification](phase-5-overload-report.md) includes the reproduced regression and complete PROTO-05 scenario.

CloseWrite sends a FIN after preceding request bytes; the destination can read request EOF and then generate a response on the still-open reverse direction. The caller reads through the peer's FIN/EOF to receive that response. Canceling one open context or abandoning its raw carrier closes that paired stream in both directions, without terminating other streams owned by the shared runtime or replaying application bytes. [Lifecycle verification](phase-5-lifecycle-report.md) exercises these endings concurrently with distinct ordered payloads and requires a response generated only after destination request EOF.
