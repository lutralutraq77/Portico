...
func TestVersionThreeMigrationPreservesFactorsAndInvalidatesChallenges(t *testing.T) {
	for _, reject := range []bool{false, true} {
		t.Run(map[bool]string{false: "migrate", true: "rollback"}[reject], func(t *testing.T) {
			...
			_, e = s.db.Exec("DROP TABLE admin_renewals; DROP TABLE session_closure_receipts; DROP TABLE session_cancellations; DROP TABLE authorized_sessions; DROP TABLE policy_previews; DROP TABLE policy_meta; PRAGMA user_version=3")
			must(t, e)
